...
export const getUserWatchedVideos = async (userId, token) => {
  const operationsDoc = `
    query getUserWatchedVideos($userId: String!) {
      stats(where: {
        watched: {_eq: true}, 
        userId: {_eq: $userId}
      }) {
  ...
  const response = await queryHasuraGraphQL(operationsDoc, "getUserWatchedVideos", { userId }, token);
